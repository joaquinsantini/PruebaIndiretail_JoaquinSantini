...
# Sumo la columna de código raíz del producto a la tabla de movimientos de stock
query = """
SELECT  m.*,
        p.ProductRootCode
        FROM df_stock_movements m
        INNER JOIN df_products p ON m.ProductId = p.ProductId"""

df_stock_movements = spark.sql(query)
df_stock_movements.createOrReplaceTempView('df_stock_movements')
...
# Agrupo por código raíz del producto, tienda y fecha y genero la nueva tabla
query = """
SELECT  ProductRootCode,
        StoreId,
        Date,
        SUM(Quantity) AS Quantity
        FROM df_stock_movements s
        GROUP BY ProductRootCode, StoreId, Date"""

df_stock_agg = spark.sql(query)
...
# Genero la columna stock generando la ventana agrupada por código raíz de producto y tienda, ordenado por fecha
query = """
SELECT  *,
        SUM(Quantity) OVER(PARTITION BY ProductRootCode, StoreId ORDER BY Date) AS Stock
        FROM df_stock_agg"""
...
SELECT  ProductRootCode,
        ...
        GROUP BY ProductRootCode, StoreId"""

df_stock_max_fecha = spark.sql(query)
df_stock_max_fecha.createOrReplaceTempView('df_stock_max_fecha')
...
query = """
SELECT  s1.ProductRootCode,
        s1.StoreId,
        CAST('2019-01-01' AS DATE) AS Date,
        s1.Stock
        FROM df_stock_agg s1
        INNER JOIN df_stock_max_fecha s2 ON s1.ProductRootCode = s2.ProductRootCode AND s1.StoreId = s2.StoreId AND s1.Date = s2.Date
UNION
SELECT  ProductRootCode,
        StoreId,
        Date,
...
# Con los registros menores o iguales al 31/12/2020, particiono por código raíz de producto y tienda para enumerar los registros de cada partición
query = """
SELECT  *,
        ROW_NUMBER() OVER(PARTITION BY ProductRootCode, StoreId ORDER BY Date) AS num_registro
        FROM df_stock_agg
...
SELECT  s1.ProductRootCode,
        s1.StoreId,
        s1.Date AS StartDate,
        IFNULL(s2.Date - INTERVAL 1 DAYS, CAST('2020-12-31' AS DATE)) AS EndDate,
        s1.Stock
        FROM df_stock_agg s1
        LEFT JOIN df_stock_agg s2 ON s1.ProductRootCode = s2.ProductRootCode AND s1.StoreId = s2.StoreId AND s1.num_registro = s2.num_registro - 1
        WHERE s1.Stock > 0"""

df_interval_stock = spark.sql(query)
...
# MAGIC %md
# MAGIC #### NOTA: Se tiene la suposición que en el FS de Databricks hay al menos 21MB de memoria libres para que los archivos puedan ser generados, sino la siguiente celda dará error
